...
export function InputSelect({
  ...
  const CommandSelect = memo(() => {
    return (
      <Command ref={commandRef} className="relative" loop>
        <div
          className={cn(
            "group relative rounded-md border border-gray-200 bg-white px-1 transition-all focus-within:border-gray-500 focus-within:ring-4 focus-within:ring-gray-200",
            className,
          )}
        >
          <div
        ...
        {openCommandList && (
          <Command.List
            className={cn("dub-scrollbar p-2", {
              "absolute z-20 mt-2 h-[calc(var(--cmdk-list-height)+17px)] max-h-[300px] w-full min-w-[160px] overflow-auto rounded-md border border-gray-200 bg-white shadow-md transition-all duration-75":
                !isMobile,
              "h-[70vh] overflow-y-auto": isMobile,
            })}
          >
            {items.map((item) => (
              <Command.Item
